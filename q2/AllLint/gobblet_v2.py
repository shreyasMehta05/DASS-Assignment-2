...
class Piece:
    ...
    def draw(self, screen, x, y, highlight_bool=True):
        ...
        if self.selected:
            ...
            pygame.draw.circle(screen, (*highlight_color, 150), (x, y), glow_radius)
        
        # Add hover effect
        if self.hover:
            hover_radius = self.radius + 2
            pygame.draw.circle(screen, (*highlight_color, 100), (x, y), hover_radius)
        
        # Draw the main piece
        pygame.draw.circle(screen, highlight_color, (x, y), self.radius)
        pygame.draw.circle(screen, border_color, (x, y), self.radius, 4)  # Border
...
class Board:
    ...
    def place_piece(self, piece: Piece, row: int, col: int) -> bool:
        ...
        # Check if the cell is empty or the new piece is larger than the top piece
        if self.cells[row][col] and self.cells[row][col][-1].size >= piece.size:
            return False
        
        # Update piece position and add to the cell
    ...
    def draw(self, screen):
        ...
        # Create a transparent surface for the board grid
        board_surface = pygame.Surface((BOARD_SIZE * CELL_SIZE, BOARD_SIZE * CELL_SIZE), pygame.SRCALPHA)
        ...
        for row in range(BOARD_SIZE):
            for col in range(BOARD_SIZE):
                ...
                # Highlight the cell if it's the current highlighted cell
                if self.highlight_cell and self.highlight_cell == (row, col):
                    pygame.draw.rect(board_surface, (130, 100, 180, 80), rect)
                
                # Outer grid lines (purple glow)
                pygame.draw.rect(board_surface, (130, 100, 180), rect, 5)  # Purple glow
                pygame.draw.rect(board_surface, (100, 80, 140), rect, 3)  # Faded purple layer
                
                # Inner grid lines
                rect_inner = rect.copy()
                rect_inner.inflate_ip(-5, -5)  # Shrink the rect for inner grid lines
                pygame.draw.rect(board_surface, (100, 80, 140, 150), rect_inner, 2)  # Soft purple inner grid

        # Blit the transparent board on top of the background
        screen.blit(board_surface, (BOARD_OFFSET, BOARD_OFFSET))

        # Draw the pieces in each cell
        for row in range(BOARD_SIZE):
            for col in range(BOARD_SIZE):
                if self.cells[row][col]:
                    piece = self.cells[row][col][-1]  # Draw only the top piece
    ...
    def check_win(self, player: int) -> Optional[List[Tuple[int, int]]]:
        ...
        Returns the winning line coordinates if there's a win, None otherwise.
        """
        # Check rows
        for row in range(BOARD_SIZE):
            if all(self.cells[row][col] and self.cells[row][col][-1].player == player 
                   for col in range(BOARD_SIZE)):
        ...
        if all(self.cells[i][i] and self.cells[i][i][-1].player == player 
               for i in range(BOARD_SIZE)):
            return [(i, i) for i in range(BOARD_SIZE)]
        
        # Check diagonal (top-right to bottom-left)
        if all(self.cells[i][BOARD_SIZE - 1 - i] and 
               self.cells[i][BOARD_SIZE - 1 - i][-1].player == player 
               for i in range(BOARD_SIZE)):
...
class Game:
    ...
    def __init__(self):
        ...
        # Initialize button
        self.restart_button = pygame.Rect(SCREEN_WIDTH//2 - 80, SCREEN_HEIGHT - 120, 160, 40)
        self.button_hover = False
    
    def _create_player_pieces(self, player: int, color: Tuple[int, int, int]) -> List[Piece]:
        """Create initial pieces for a player."""
        pieces = []
        # 2 of each size per player
    ...
    def handle_click(self, pos: Tuple[int, int]):
        ...
        board_rect = pygame.Rect(BOARD_OFFSET, BOARD_OFFSET, 
        ...
        # Check if click is on the player's reserve
    ...
    def _handle_board_click(self, row: int, col: int):
        ...
        # If no piece is selected and we clicked on our own piece
    ...
    def _handle_reserve_click(self, pos: Tuple[int, int]):
        ...
        reserve = self.player1_reserve if self.current_player == 1 else self.player2_reserve
        reserve_x = RESERVE_OFFSET_X if self.current_player == 1 else SCREEN_WIDTH - RESERVE_OFFSET_X
        
        for i, piece in enumerate(reserve):
            # Calculate piece position in reserve
            piece_y = RESERVE_START_Y + i * RESERVE_SPACING
            
            # Check if this piece was clicked
            distance = ((pos[0] - reserve_x) ** 2 + (pos[1] - piece_y) ** 2) ** 0.5
            if distance <= piece.radius + 10:  # Adding a bit of margin for easier selection
                # If we already have a selected piece, deselect it
                if self.selected_piece:
                    self.selected_piece.selected = False
                    # If it's from the board, put it back
                    ...
                    # If it's from reserve, add it back
                    else:
    ...
    def handle_motion(self, pos: Tuple[int, int]):
        ...
        # Check for reserve piece hover
        for player in [1, 2]:
            reserve = self.player1_reserve if player == 1 else self.player2_reserve
    ...
    def draw(self, screen):
        ...
        # Draw player's reserves with labels
        self._draw_reserve(screen)
        
        # Draw the selected piece under the cursor if any
        if self.selected_piece:
            mouse_x, mouse_y = pygame.mouse.get_pos()
            self.selected_piece.draw(screen, mouse_x, mouse_y)
        
        # Draw current player indicator with appropriate color
        ...
        if self.game_over:
            ...
            pygame.draw.rect(screen, (30, 100, 30), self.restart_button, 3, border_radius=10)
    ...
    def _draw_reserve(self, screen):
        ...
        screen.blit(p1_label, (RESERVE_OFFSET_X - p1_label.get_width()//2 + 60, RESERVE_START_Y - 90))
        screen.blit(p2_label, (SCREEN_WIDTH - RESERVE_OFFSET_X - p2_label.get_width()//2 - 60, RESERVE_START_Y - 90))
        
        # Draw player 1's reserve (left side)
        for i, piece in enumerate(self.player1_reserve):
            piece_x = RESERVE_OFFSET_X
            piece_y = RESERVE_START_Y + i * RESERVE_SPACING
            # Draw piece background/holder
            pygame.draw.circle(screen, (60, 60, 60, 150), (piece_x, piece_y), piece.radius + 5)
            # Draw the piece
            piece.draw(screen, piece_x, piece_y, self.current_player == 1)
        
        # Draw player 2's reserve (right side)
    ...
    def _draw_winning_line(self, screen):
        ...
        # Create a surface for the winning line overlay
        line_surface = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA)
        
        # Get the center coordinates of the first and last cell in the winning line
        ...
        # Draw a pulse effect for each winning cell
        ...
        for row, col in self.winning_line:
            ...
            pygame.draw.circle(line_surface, (*highlight_color, 100), (cell_x, cell_y), CELL_SIZE//2 - pulse_size)
        
        # Draw line connecting the winning cells
        pygame.draw.line(line_surface, (*highlight_color, 180), (start_x, start_y), (end_x, end_y), 8)

...
def load_background():
    ...
    try:
        bg = pygame.image.load("pic/game-background.jpg").convert()  # pylint: disable=no-member
        return pygame.transform.scale(bg, (SCREEN_WIDTH, SCREEN_HEIGHT))
    except pygame.error:  # pylint: disable=no-member
        print("Warning: Could not load background image. Using fallback.")
        fallback = pygame.Surface(SCREEN_SIZE, pygame.SRCALPHA)  # pylint: disable=no-member
...
class Piece:  # pylint: disable=too-few-public-methods
    ...
    def draw(self, surface, x, y, highlight_bool=True):
        """
        ...
        if self.selected:
            ...
            pygame.draw.circle(surface, (*highlight_color, 150), (x, y), glow_radius)
        if self.hover:
            hover_radius = self.radius + 2
            pygame.draw.circle(surface, (*highlight_color, 100), (x, y), hover_radius)
        pygame.draw.circle(surface, highlight_color, (x, y), self.radius)
        pygame.draw.circle(surface, border_color, (x, y), self.radius, 4)
        if self.size == 1:
            pygame.draw.circle(surface, WHITE, (x, y), 3)
        elif self.size == 2:
            pygame.draw.circle(surface, WHITE, (x - 5, y), 3)
            pygame.draw.circle(surface, WHITE, (x + 5, y), 3)
        elif self.size == 3:
            pygame.draw.circle(surface, WHITE, (x, y), 3)
            pygame.draw.circle(surface, WHITE, (x - 7, y), 3)
            pygame.draw.circle(surface, WHITE, (x + 7, y), 3)
    ...
    def draw(self, surface):
        ...
        for row in range(BOARD_SIZE):
            for col in range(BOARD_SIZE):
                ...
                pygame.draw.rect(board_surface, (100, 80, 140, 150), rect_inner, 2)
        surface.blit(board_surface, (BOARD_OFFSET, BOARD_OFFSET))
        for row in range(BOARD_SIZE):
            for col in range(BOARD_SIZE):
                if self.cells[row][col]:
                    ...
                    piece.draw(surface, piece_x, piece_y)
...
class Game:  # pylint: disable=too-many-instance-attributes
    ...
    def draw(self, surface):
        ...
        self._draw_reserve(surface)
        if self.selected_piece:
            mouse_x, mouse_y = pygame.mouse.get_pos()
            self.selected_piece.draw(surface, mouse_x, mouse_y)
        ...
        if self.game_over:
            ...
            pygame.draw.rect(surface, button_color, self.restart_button, border_radius=10)
            pygame.draw.rect(
                surface, (30, 100, 30), self.restart_button, 3, border_radius=10
            )
    ...
    def _draw_reserve(self, surface):
        ...
        for i, piece in enumerate(self.player1_reserve):
            ...
            piece_y = RESERVE_START_Y + i * RESERVE_SPACING
            pygame.draw.circle(
                surface, (60, 60, 60, 150), (piece_x, piece_y), piece.radius + 5
            )
            piece.draw(surface, piece_x, piece_y, self.current_player == 1)
        for i, piece in enumerate(self.player2_reserve):
            ...
            piece.draw(surface, piece_x, piece_y, self.current_player == 2)

    def _draw_winning_line(self, surface):
        """Draw a line through the winning cells."""
        line_surface = pygame.Surface(
            (SCREEN_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA  # pylint: disable=no-member
        )
        ...
        # Split this long call into two lines to fix the line-length issue:
        pygame.draw.line(
            line_surface,
            (*highlight_color, 180),
            (start_x, start_y),
